...
function DataManagementContent() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">This page is restricted to super admins.</p>
        </div>
      </div>
    );
  }
    ...
    async function loadGuilds() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const opts: GuildOption[] = Array.isArray(data)
            ? data.map((g: any) => ({ id: g.id, name: g.name, nickname: g.nickname }))
            : [];
          setGuilds(opts);
          if (opts.length > 0) setSelectedGuildId(opts[0].id);
        }
      } catch {
        // ignore
      } finally {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [activityRes, challengeRes] = await Promise.all([
        api.get('/api/activity/history', { guildId: selectedGuildId }),
        api.get('/api/challenges/list', { guildId: selectedGuildId }),
      ]);

      if (activityRes.ok) setActivityData(await activityRes.json());
      if (challengeRes.ok) setChallengeData(await challengeRes.json());
    } catch (err) {
  ...
  useEffect(() => {
    if (selectedGuildId) loadData();
  }, [selectedGuildId, loadData]);
  ...
  function selectAllActivity() {
    if (selectedActivityDates.size === activityData.length) setSelectedActivityDates(new Set());
    else setSelectedActivityDates(new Set(activityData.map(d => d.date)));
  }

  function selectAllChallenges() {
    if (selectedChallengeDates.size === challengeData.length) setSelectedChallengeDates(new Set());
    else setSelectedChallengeDates(new Set(challengeData.map(c => c.challenge_date)));
  }

  async function deleteSelectedActivity() {
    if (selectedActivityDates.size === 0) return;
    if (!window.confirm(`Delete activity logs for ${selectedActivityDates.size} date(s)? Cannot be undone.`)) return;

    setDeleting(true);
    try {
      const res = await api.post('/api/activity/delete', { dates: Array.from(selectedActivityDates) }, { guildId: selectedGuildId });
      if (res.ok) {
        await loadData();
      } else {
  ...
  async function deleteSelectedChallenges() {
    ...
    if (!window.confirm(`Delete ${selectedChallengeDates.size} challenge(s)? Cannot be undone.`)) return;

    setDeleting(true);
    try {
      const res = await api.post('/api/challenges/delete', { dates: Array.from(selectedChallengeDates) }, { guildId: selectedGuildId });
      if (res.ok) {
        await loadData();
      } else {
        const error = await res.json();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">View, edit, and delete activity logs and challenges across all guilds</p>
        <p className="text-xs text-muted-foreground mt-1">
          Note: Members meet requirements by reaching the configured total gold (challenge donations + valid guild hall deposits).
        </p>
      </div>
            ...
            <SelectContent>
              {guilds.map(g => (
                <SelectItem key={g.id} value={g.id}>
                  <span className="font-mono font-bold text-primary">{g.nickname}</span>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {activityData.length} date(s) — {selectedGuild?.nickname ?? selectedGuildId}
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={selectAllActivity}>
                      ...
                      {expandedDate === dayData.date && (
                        <div className="border-t bg-muted/30 p-4">
                          <div className="mb-3 p-2 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded text-xs text-blue-700 dark:text-blue-300">
                            <strong>Editing Note:</strong> When editing gold values, requirement status is recalculated based only on gold threshold.
                          </div>
                                  ...
                                  <td className="py-2 text-right">
                                    {editingLog?.id === log.id ? (
                                      <div className="flex gap-1 justify-end">
                                        <Button size="sm" variant="outline" onClick={saveEditLog}>Save</Button>
                                        <Button size="sm" variant="ghost" onClick={() => setEditingLog(null)}>Cancel</Button>
                                      </div>
                                    ) : (
                                      <Button
                                        size="sm"
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {challengeData.length} challenge(s) — {selectedGuild?.nickname ?? selectedGuildId}
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={selectAllChallenges}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {editingChallenge?.id === challenge.id ? (
                          <div className="flex gap-1">
                            <Button size="sm" variant="outline" onClick={saveEditChallenge}>Save</Button>
                            <Button size="sm" variant="ghost" onClick={() => setEditingChallenge(null)}>Cancel</Button>
                          </div>
                        ) : (
                          <Button
                            size="sm"
